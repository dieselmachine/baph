...
def parse_metadump(line):
    ...
    data = dict(list(map(KEY_VALUE_SPLITTER, line)))
...
class MemcacheServer(object):
    ...
    def get_metadump(self, slab_id):
        ...
        return list(map(parse_metadump, self.send_command(cmd)))
    ...
    def get_stats(self, *args):
        ...
        """
        cmd = ' '.join(('stats',) + args)
        return list(map(parse_stat, self.send_command(cmd, 3)))
    ...
    def get_cachedump(self, slab_id, limit):
        ...
        return list(map(parse_cachedump, self.get_stats(cmd)))
...
class BaphMemcachedCache(MemcachedCache):
    ...
    @cached_property
    def servers(self):
        return list(map(MemcacheServer, self._cache.servers))

    def get_all_keys(self):
        keys = set()
        for server in self.servers:
            try:
                del server.slabs  # force rebuild of slab ids if cached
